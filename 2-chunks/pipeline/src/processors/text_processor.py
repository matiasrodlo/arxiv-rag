...
class TextProcessor:
    ...
    def extract_sections(self, text: str, pages: Optional[List[Dict]] = None) -> List[Dict[str, any]]:
        ...
        DEBUG_LOG_PATH = Path("/Volumes/8SSD/DOIS/.cursor/debug.log")
                    ...
                    if match:
                        ...
                        if current_section['content_lines']:
                            ...
                            if section_text:  # Only add non-empty sections
                                # Calculate end_char as start_char + text length
                                section_start = current_section['start_char']
                            ...
                            # Start new section (char_position is where the section header starts)
                            # The actual content starts after the header line
                            current_section = {
                                'name': section_name,
                                'text': '',
                                'start_char': char_position + len(line) + 1,  # Start after header line
                                'content_lines': []
                            }
                    ...
                    if not is_section_header:
                        for key, normalized_name in fuzzy_sections.items():
                            if line_lower == key or line_lower.startswith(key + ' ') or line_lower.endswith(' ' + key):
                                ...
                                # Start new section with normalized name (content starts after header)
                                current_section = {
    ...
    def extract_metadata(text: str, sections: Optional[List[Dict]] = None) -> Dict:
        ...
        # Select best title candidate (first one found)
        if title_candidates:
        ...
        if sections:
            for section in sections:
                ...
                if 'abstract' in section_name:
                    abstract_text = section.get('text', '').strip()
                    break
        
        # If no abstract in sections, search in text
        ...
        if abstract_text:
            ...
            abstract_text = re.sub(r'^arXiv:\s*\d+\.\d+v?\d*\s*', '', abstract_text)
            metadata['abstract'] = abstract_text[:2000]  # Limit length
        else:
            metadata['abstract'] = None
        
        # Extract authors (usually before abstract or in first lines)
        authors = []
        author_patterns = [
            r'Authors?[:\s]+([^\n]+)',
            r'By\s+([^\n]+)',
...
class TextChunker:
                ...
                if device == "mps":
                    try:
                        ...
                        if is_worker_process or not mps_available:
                            ...
                            # Suppress warning - this is expected behavior
                            actual_device = "cpu"
    ...
    def _chunk_fixed(self, text: str, metadata: Optional[Dict] = None) -> List[Dict[str, any]]:
        """Fixed-size chunking with improved boundary detection."""
        # #region agent log
        ...
        def find_best_boundary(text: str, start_pos: int, target_end: int, min_size: int) -> int:
            """
            Find the best chunk boundary near target_end.
            Prioritizes: sentence endings > word boundaries > target position
            """
            if target_end >= len(text):
                return len(text)
            
            # Look back up to 30% of chunk size for better boundaries
            lookback_size = int(self.chunk_size * 0.3)
            lookback_start = max(start_pos, target_end - lookback_size)
            search_region = text[lookback_start:target_end]
            
            # Priority 1: Sentence endings (with proper spacing)
            # Patterns: . ! ? followed by space/newline
            sentence_patterns = [
                r'\.\s+',      # Period + space
                r'\.\n+',      # Period + newline
            ...
            for pattern in sentence_patterns:
                ...
                if matches:
                    # Use the last (closest to target) match
                    match = matches[-1]
                    candidate = lookback_start + match.end()
                    # Ensure it's not too small
                    if candidate - start_pos >= min_size:
                        best_sentence_end = max(best_sentence_end, candidate)
            
            if best_sentence_end > lookback_start:
                return best_sentence_end
            
            # Priority 2: Word boundaries (space or newline)
            # Look for word boundary near target
            ...
            for pattern in word_boundary_patterns:
                ...
                if matches:
                    ...
                    # Ensure it's not too small
                    if candidate - start_pos >= min_size:
                        best_word_end = max(best_word_end, candidate)
            
            if best_word_end > lookback_start:
                return best_word_end
            
            # Priority 3: Use target position (fallback)
            return target_end
        ...
        while start < text_length and iteration < max_iterations:
            ...
            # Find best boundary
            end = find_best_boundary(text, start, target_end, self.min_chunk_size)
            chunk_text = text[start:end]
            
            # Ensure chunk ends properly (trim trailing whitespace but preserve structure)
            chunk_text = chunk_text.rstrip()
            
            # Final check: if chunk doesn't end with punctuation and we're not at text end,
            # try to extend slightly to find sentence end (up to 200 chars)
            if end < text_length and chunk_text and chunk_text[-1] not in '.!?;':
                # Look ahead up to 200 chars for sentence ending
                lookahead_end = min(end + 200, text_length)
                lookahead_text = text[end:lookahead_end]
                
                # Find first sentence ending
                sentence_match = re.search(r'[.!?]\s+', lookahead_text)
                if sentence_match:
                    # Extend chunk to include complete sentence
                    new_end = end + sentence_match.end()
                    extended_text = text[start:new_end].rstrip()
                    # Only extend if it doesn't exceed max_chunk_size too much
                    if len(extended_text) <= self.max_chunk_size:
                        end = new_end
